...
class SettingController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = Setting::active()->get();
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action', function($row){
                        $btn = '';
                        //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                           $btn = $btn.'<a style="margin:5px 5px" href="'.action('Setting\SettingController@edit',$row->id).'" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
    ...
    {
        return view('setting.edit', ['data' => Setting::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if($request->ajax()){
            $result = Setting::find($request->id)->update([
                'metaname' => $request->metaname,
