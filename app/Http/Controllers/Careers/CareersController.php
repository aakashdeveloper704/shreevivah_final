...
class CareersController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = Careers::active()->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $btn = '';
                    //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                    $btn = $btn . '<a style="margin:5px 5px" href="' . action('Careers\CareersController@edit', $row->id) . '" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
        ...
        // return view('setting.index', ['rows' => Setting::active()->paginate(20)]);
    }
    ...
    {
        return view('careers.edit', ['data' => Careers::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax()) {
            $result = Careers::find($request->id)->update([
                'metaname' => $request->metaname,
                'metavalue' => $request->metavaluenew,
                'active' => $request->active,
            ]);
            return $result ? response()->json([
                'code' => 200,
    ...
    public function delete(Request $request)
    {
        if ($request->ajax()) {
            ...
            return $result ? response()->json([
                'code' => 200,
                'error' => false,
                'message' => 'Setting Info Delete Successfully!',
            ]) : response()->json([
                'code' => 401,
                'error' => true,
                'message' => 'Something went wrong!',
