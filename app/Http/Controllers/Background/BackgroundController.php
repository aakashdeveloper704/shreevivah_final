...
class BackgroundController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Background::active()->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $btn = '';
                    //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                    $btn = $btn . '<a style="margin:5px 5px" href="' . action('Background\BackgroundController@edit', $row->id) . '" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
    ...
    public function insert(Request $request)
    {
        if ($request->ajax()) {
            ...
            return $result ? response()->json([
                'code' => 200,
                'error' => false,
                'message' => 'Background page Added Successfully!',
            ]) : response()->json([
                'code' => 401,
                'error' => true,
    ...
    {
        return view('background.edit', ['data' => Background::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax()) {
            $result = Background::find($request->id)->update([
                'metaname' => $request->metaname,
                'metavalue' => $request->metavalue,
                'active' => $request->active,
            ]);
