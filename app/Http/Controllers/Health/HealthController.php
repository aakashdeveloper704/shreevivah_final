...
class HealthController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Health::active()->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $btn = '';
                    //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                    $btn = $btn . '<a style="margin:5px 5px" href="' . action('Health\HealthController@edit', $row->id) . '" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
    ...
    public function add()
    {
        return view('health.add');
    }

    public function insert(Request $request)
    {
        if ($request->ajax()) {
            $result = Health::insert([
            ...
            return $result ? response()->json([
                'code' => 200,
                'error' => false,
                'message' => 'Health page Added Successfully!',
            ]) : response()->json([
                'code' => 401,
                'error' => true,
    ...
    {
        return view('health.edit', ['data' => Health::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax()) {
            $result = Health::find($request->id)->update([
                'metaname' => $request->metaname,
                'metavalue' => $request->metavalue,
                'active' => $request->active,
            ]);
    ...
    public function delete(Request $request)
    {
        if ($request->ajax()) {
            ...
            return $result ? response()->json([
                'code' => 200,
                'error' => false,
                'message' => 'Health Delete Successfully!',
            ]) : response()->json([
                'code' => 401,
                'error' => true,
                'message' => 'Something went wrong!',
