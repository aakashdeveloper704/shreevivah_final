...
class DisclaimerController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Disclaimer::active()->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $btn = '';
                    //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                    $btn = $btn . '<a style="margin:5px 5px" href="' . action('Disclaimer\DisclaimerController@edit', $row->id) . '" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
    ...
    public function add()
    {
        return view('disclaimer.add');
    }

    public function insert(Request $request)
    {
        if ($request->ajax()) {
            $result = Disclaimer::insert([
            ...
            return $result ? response()->json([
                'code' => 200,
                'error' => false,
                'message' => 'Disclaimer page Added Successfully!',
            ]) : response()->json([
                'code' => 401,
                'error' => true,
    ...
    {
        return view('disclaimer.edit', ['data' => Disclaimer::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax()) {
            $result = Disclaimer::find($request->id)->update([
                'metaname' => $request->metaname,
                'metavalue' => $request->metavalue,
                'active' => $request->active,
            ]);
    ...
    public function delete(Request $request)
    {
        if ($request->ajax()) {
            ...
            return $result ? response()->json([
                'code' => 200,
                'error' => false,
                'message' => 'Disclaimer Delete Successfully!',
            ]) : response()->json([
                'code' => 401,
                'error' => true,
                'message' => 'Something went wrong!',
