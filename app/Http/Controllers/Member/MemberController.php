...
class MemberController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Member::active()->get();
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action', function($row){
                        $btn = '';
                        //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                           $btn = $btn.'<a style="margin:5px 5px" href="'.action('Member\MemberController@edit',$row->id).'" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
        ...
        // return view('member.index', ['rows' => Member::active()->paginate(20)]);
    }
    ...
    {
        return view('member.edit', ['data' => Member::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if($request->ajax()){
            $result = Member::find($request->id)->update([
                'metaname' => $request->metaname,
                'metavalue' => $request->metavalue,
                'active' => $request->active,
            ]);
