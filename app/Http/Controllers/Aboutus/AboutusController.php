...
class AboutusController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = About_us::active()->get();
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action', function($row){
                        $btn = '';
                        //    $btn = '<a href= class="edit btn btn-info btn-sm">View</a>';
                           $btn = $btn.'<a style="margin:5px 5px" href="'.action('Aboutus\AboutusController@edit',$row->id).'" class="edit btn btn-primary btn-sm pd-5">Edit</a>';
        ...
        // return view('aboutus.index', ['rows' => Aboutus::active()->paginate(20)]);
    }
    ...
    {
        return view('aboutus.edit', ['data' => About_us::find($id)]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if($request->ajax()){
            $result = About_us::find($request->id)->update([
                'metaname' => $request->metaname,
                'metavalue' => $request->metavalue,
                'active' => $request->active,
            ]);
    ...
    public function delete(Request $request)
    {
        ...
        return $result ? response()->json([
            'code'=>200,
            'error'=>false,
            'message'=>'Aboutus Info Delete Successfully!',
        ]) : response()->json([
            'code'=>401,
            'error'=>true,
            'message'=>'Something went wrong!',
