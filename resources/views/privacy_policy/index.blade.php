    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="d-flex border-bottom title-part-padding align-items-center">
                <div>
                    <h4 class="card-title mb-0">Privacy Policy<a href="{{route('privacy_policy.add')}}" class="btn btn-rounded btn-primary">Add</a></h4>
                </div>
            </div>
            <div class="card-body">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        users_table = $('#dataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('privacy_policy.index')}}",
            columnDefs: [{
                "targets": [1],
                "orderable": false,
                "searchable": false
    ...
    function delete_function(id) {
        swal({
            ...
            if (willDelete) {
                var href = "{{route('privacy_policy.delete')}}";
                var data = id;
                $.ajax({
                    headers: {
